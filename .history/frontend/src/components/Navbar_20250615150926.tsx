...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav className="bg-white shadow-sm border-b border-gray-200 py-3 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3 ml-auto">
            ...
            <FaSearch
              className="text-gray-800 text-xl cursor-pointer hover:scale-110 transition"
      ...
      {/* Search Overlay */}
      {isSearchOpen && (
        <div className="fixed inset-0 z-50 bg-white flex items-start justify-center pt-20 px-4">
          <div className="relative w-full max-w-2xl">
            {/* Close Button */}
            <button
              onClick={() => setIsSearchOpen(false)}
              className="absolute top-[-60px] right-0 text-3xl text-gray-800"
            >
              <FaTimes />
            </button>
            ...
            <input
              ...
              placeholder="Search courses..."
              autoFocus
              className="w-full border border-gray-300 rounded-md px-4 py-3 text-lg focus:outline-none focus:ring-2 focus:ring-gray-800"
            />
          </div>
        </div>
      )}
