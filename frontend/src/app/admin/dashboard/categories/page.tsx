...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    fetchCategoriesFiltered(searchTerm, parentCategoryFilter);
  }, [fetchCategoriesFiltered, searchTerm, parentCategoryFilter]);
      ...
      <div className="overflow-y-auto max-h-[500px]">
        {categories.length === 0 && searchTerm.trim() !== "" ? (
          <div className="flex items-center justify-center h-48 text-gray-500 italic text-lg">
            😕 No categories found matching "<span className="font-semibold">{searchTerm}</span>"
          </div>
        ) : (
          <table className="min-w-full text-sm text-left border-collapse">
            <thead className="bg-gray-100 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-6 py-3 border-b border-gray-300">Parent</th>
                <th className="px-6 py-3 border-b border-gray-300 text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {categories.map((cat) => (
  ...
  <CreateCategoryModal isOpen={openCreate} onClose={() => setOpenCreate(false)} />
  <EditCategoryModal
    isOpen={openEdit}
